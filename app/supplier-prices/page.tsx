...
export default function SupplierPricesPage() {
	...
	const handleAddSupplier = () => {
		if (!name || !ingredient || !prevPrice || !currentPrice)
	...
	const toggleReason = (id: string) =>
		setShowReasonId(showReasonId === id ? null : id);
	...
	const getCustomReasons = (ingredientName: string) => {
		...
		if (lower.includes("rice") || lower.includes("beras")) {
			return [
				{
					...
					text: "Climate Factors: Droughts in Southeast Asia reducing paddy yields.",
				},
				{
					icon: <Globe size={14} />,
					text: "Import Dependency: Export restrictions from major global suppliers (e.g. India).",
				},
			];
		} else if (
			...
			return [
				{
					icon: <Globe size={14} />,
					text: "Feed Costs: Global spikes in imported corn and soybean prices affecting poultry feed.",
				},
				{
					icon: <Truck size={14} />,
					text: "Logistics: Higher diesel costs impacting transport from rural farms.",
				},
			];
		} else if (
			lower.includes("veg") ||
			...
			return [
				{
					icon: <CloudRain size={14} />,
					text: "Weather Extremes: Monsoon rains damaging highland crop yields (e.g. Cameron Highlands).",
				},
				{
					icon: <Globe size={14} />,
					text: "Fertilizer Costs: Currency exchange rates increasing the price of imported agricultural chemicals.",
				},
			];
		} else {
			return [
				{
					icon: <Globe size={14} />,
					text: "Macroeconomics: General food inflation and currency fluctuations.",
				},
				{
					icon: <Truck size={14} />,
					text: "Supply Chain: Increased fuel and distribution costs nationwide.",
				},
			];
		}
	};
	...
	if (!isLoaded)
		return <div className="p-10 bg-[#FFFCF6] min-h-screen">Loading...</div>;
		...
		<div className="p-10 bg-[#FFFCF6] min-h-screen font-sans">
			<header className="mb-10">
				...
				<p className="text-gray-500 text-lg">
					Track ingredient inflation, margin impact, and global factors
				</p>
			</header>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
				<div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm lg:col-span-2">
					...
					<div className="space-y-4">
						<div className="grid grid-cols-2 gap-4">
							<div>
								...
								<input
									...
									type="text"
									value={name}
									aria-labelledby="supplier-name"
									onChange={(e) => setName(e.target.value)}
									placeholder="e.g. Borneo Agri-Coop"
									className="w-full p-3 bg-blue-50 rounded-lg border border-blue-100 outline-none focus:outline-solid"
								/>
							</div>
							<div>
								<label
								...
								<input
									...
									placeholder="e.g. White Rice (10kg)"
									className="w-full p-3 bg-blue-50 rounded-lg border border-blue-100 outline-none focus:outline-solid"
								/>
							</div>
						</div>
						...
						<button
							...
							className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition mt-2 flex justify-center items-center gap-2"
						>
				...
				<div className="bg-purple-50 p-8 rounded-2xl border-2 border-dashed border-purple-200 shadow-sm flex flex-col items-center justify-center text-center relative hover:border-purple-400 hover:bg-purple-100 transition-all group">
					...
					<p className="text-xs text-purple-600 mb-4 px-2">
						Upload your market spreadsheet to instantly calculate margin
						impacts.
					</p>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{suppliers.map((supplier) => (
					<div
						key={supplier.id}
						className="p-6 rounded-2xl border border-gray-100 bg-white shadow-sm flex flex-col hover:border-blue-200 transition-all"
					>
						...
						<div className="flex items-center gap-4 mb-4">
							<div>
								...
								<p className="text-lg font-semibold text-gray-500 line-through">
									RM{supplier.prevPrice.toFixed(2)}
								</p>
							</div>
							...
							<div>
								...
								<p className="text-2xl font-black text-gray-900">
									RM{supplier.currentPrice.toFixed(2)}
								</p>
							</div>
						</div>

						<div
							className={`p-3 rounded-xl border flex items-center justify-between mb-4 ${supplier.percentIncrease > 0 ? "bg-red-50 border-red-100 text-red-700" : "bg-green-50 border-green-100 text-green-700"}`}
						>
							<div className="flex items-center gap-2">
								{supplier.percentIncrease > 0 ? (
									<TrendingUp size={18} />
								) : (
									<TrendingDown size={18} />
								)}
								<span className="font-bold">
									{Math.abs(supplier.percentIncrease).toFixed(1)}%{" "}
									{supplier.percentIncrease > 0 ? "Increase" : "Decrease"}
								</span>
							</div>
							<span className="text-xs font-bold">
								Margin: {supplier.marginImpact > 0 ? "-" : "+"}RM
								{Math.abs(supplier.marginImpact).toFixed(2)}
							</span>
						</div>
						...
						<div className="mt-auto">
							<button
								type="button"
								onClick={() => toggleReason(supplier.id)}
								className="w-full flex items-center justify-center gap-2 text-sm font-bold text-blue-600 bg-blue-50 py-2 rounded-lg hover:bg-blue-100 transition"
								aria-labelledby="Show AI reason"
							>
