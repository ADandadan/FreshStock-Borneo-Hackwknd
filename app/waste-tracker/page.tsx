...
import { DollarSign, Sparkles, Sprout, Trash2 } from "lucide-react";
import { useEffect, useState } from "react";
import type { Product, WasteLog } from "@/constants";
...
export default function WasteTrackerPage() {
	...
	const selectedProduct =
		products.find((p) => p.id === Number(selectedProductId)) || null;
	...
	useEffect(() => {
		...
		if (saved) setProducts(JSON.parse(saved));
			...
			setProducts([
				{ id: 1, name: "Fresh Tomatoes", inStock: 50, sellingPrice: 0.5 },
				{ id: 2, name: "Nasi Lemak Pre-pack", inStock: 50, sellingPrice: 2 },
			]);
	}, []);
	...
	const handleLogWaste = async () => {
		...
		if (!product || !quantity || !reason || !calculatedCost || !totalBatchStock)
		...
		if (qty > product.inStock)
			return alert(
				`Waste quantity cannot exceed stock (${product.inStock} units)`,
			);
	...
	const totalFinancialLoss = wasteLogs.reduce(
		(sum, log) => sum + log.costLost,
		0,
	);
		...
		<div className="p-10 bg-[#FFFCF6] min-h-screen font-sans">
			<header className="mb-10">
				...
				<p className="text-gray-500 text-lg">
					Calculate loss, waste rates, and discover sustainable solutions
				</p>
			</header>
				...
				<div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
					...
					<div className="space-y-4">
						<select
							...
							aria-label="Select product combo box"
						>
						...
						<button
							...
							className="w-full py-3 bg-red-500 text-white rounded-xl font-bold hover:bg-red-600 transition mt-2"
						>
				...
				<div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-center">
					<div className="bg-red-50 p-6 rounded-xl border border-red-100 flex flex-col justify-center items-center text-center mb-6">
						...
						<p className="text-5xl font-black text-red-600">
							RM {totalFinancialLoss.toFixed(2)}
						</p>
					</div>
					<div className="bg-green-50 p-4 rounded-xl border border-green-100 text-sm flex gap-3 items-start">
						...
						<p className="text-green-800 font-medium">
							Turning waste into fertilizer (Buat Baja) supports local
							agriculture, creating a circular food economy right here in our
							community.
						</p>
					</div>
				</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
				{wasteLogs.map((log) => (
					<div
						key={log.id}
						className="p-6 rounded-2xl border border-gray-100 bg-white shadow-sm hover:border-red-200 transition-all"
					>
						...
						<div className="grid grid-cols-2 gap-4 mb-4">
							<div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
								...
								<p className="text-lg font-bold text-red-6  00">
									RM {log.costLost.toFixed(2)}
								</p>
							</div>
							<div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
								<p className="text-xs text-gray-500 uppercase font-bold">
