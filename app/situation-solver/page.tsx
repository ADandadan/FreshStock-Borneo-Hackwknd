...
import type { Product, Scenario } from "@/constants";
...
const SCENARIOS: Scenario[] = [
	// Climate & Natural Disaster
	{
		...
		recommendations: [
			...
			"Increase monitoring of stock movement",
		],
	},
	{
		id: "typhoon",
		...
		wasteImpactText:
			'+15% "ugly food" waste (damaged-but-edible goods rejected by consumers)',
		priceMod: 0.2,
		wasteMod: 0.15,
		riskLevel: "HIGH",
		recommendations: [
	...
	},
	{
		...
		riskLevel: "CRITICAL",
		recommendations: [
			"Implement emergency stock reduction measures immediately",
			"Consider temporary price adjustments to maintain margins",
	...
	// Economic & Policy Scenarios
	{
		...
		priceImpactText:
			"+18% increase in cost for imported ingredients (e.g., flour, dairy)",
		wasteImpactText: "No direct waste impact",
		priceMod: 0.18,
		wasteMod: 0,
		riskLevel: "HIGH",
		specialAlert: {
			title: "Food Security Literacy",
			text: "Global markets affect local breakfast prices. When currency weakens, imported goods become more expensive.",
			type: "literacy",
		},
		recommendations: [
			"Review and optimize inventory levels",
	...
	},
	{
		...
		wasteImpactText:
			"Reduced delivery frequency leads to +5% overstocking risk to avoid shortages",
		priceMod: 0.1,
		wasteMod: 0.05,
		riskLevel: "LOW",
		recommendations: [
			"Maintain current operations",
			"Use this as an opportunity to optimize processes",
	...
	},
	{
		...
		riskLevel: "CRITICAL",
		specialAlert: {
			title: "Security Score Impact",
			text: "Significant drop in community food stability",
			type: "security",
		},
		recommendations: [
			"Implement emergency stock reduction measures immediately",
	...
	// Supply Chain & Food Safety
	{
		...
		priceImpactText:
			'Immediate 10% price hike on remaining "safe" stock to cover losses',
	...
	},
	{
		...
		wasteImpactText:
			'+30% of goods arrive "near-expiry," forcing immediate 50% discounts or disposal',
		priceMod: 0.15,
		wasteMod: 0.3,
		riskLevel: "CRITICAL",
		recommendations: [
	...
	},
	{
		...
		recommendations: [
			...
			"Consider incremental price adjustments",
		],
	},
];
...
export default function SituationSolverPage() {
	...
	const calculateImpacts = (scenario: Scenario) => {
		...
		const currentTotalStock =
			products.reduce((acc, p) => acc + p.inStock, 0) || 45;
		const stockShortage = Math.ceil(currentTotalStock * scenario.wasteMod);
		const costIncrease = currentTotalStock * scenario.priceMod * 2.3;
			...
			<div className="p-10 bg-[#FFFCF6] min-h-screen font-sans">
				...
				<div className="bg-white p-6 rounded-2xl border border-orange-200 mb-4 shadow-sm">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						<div className="p-4 bg-[#FFF5F5] border border-red-100 rounded-xl">
							...
							<p className="text-xs text-red-700 leading-relaxed">
								{selectedScenario.priceImpactText}
							</p>
						</div>
						<div className="p-4 bg-[#FFFAEC] border border-yellow-200 rounded-xl">
							...
							<p className="text-xs text-yellow-700 leading-relaxed">
								{selectedScenario.wasteImpactText}
							</p>
						</div>
					</div>
					...
					{selectedScenario.specialAlert && (
						<div
							className={`mt-4 p-4 rounded-xl border flex gap-3 items-center ${
								selectedScenario.specialAlert.type === "security"
						...
						>
							...
							<div>
								...
								>
									{selectedScenario.specialAlert.title}
								</p>
								<p className="text-xs text-gray-600 leading-tight">
									{selectedScenario.specialAlert.text}
								</p>
							</div>
						</div>
					)}
				...
				<div className="bg-white p-8 rounded-3xl border border-orange-100 shadow-sm border-b-4 border-b-orange-400">
					...
					<div className="grid grid-cols-3 gap-6 mb-10">
						...
						<div className="bg-white border border-gray-50 p-6 rounded-2xl text-center shadow-sm">
							...
							<p
								className={`text-3xl font-black ${riskColors[selectedScenario.riskLevel]}`}
							>
					...
					<div className="space-y-4 mb-10 text-sm">
						<div className="flex justify-between border-b border-gray-50 pb-2">
							...
							<span className="font-bold text-gray-900">
								+RM {(parseFloat(revenueImpact) * 0.4).toFixed(2)}
							</span>
						</div>
						<div className="flex justify-between border-b border-gray-50 pb-2">
							<span className="text-gray-500">Price Impact</span>
							<span className="font-bold text-gray-900">
								+{selectedScenario.priceMod * 100}%
							</span>
						</div>
						<div className="flex justify-between border-b border-gray-50 pb-2">
							<span className="text-gray-500">Waste Increase</span>
							<span className="font-bold text-gray-900">
								+{selectedScenario.wasteMod * 100}%
							</span>
						</div>
					</div>
					...
					<div className="bg-[#FFF4E5] p-6 rounded-2xl border border-orange-100">
						<h4 className="flex items-center gap-2 text-sm font-bold text-orange-900 mb-4 italic">
							💡 Recommendations
						</h4>
		...
		<div className="p-10 bg-[#FFFCF6] min-h-screen font-sans">
			<header className="mb-10">
				...
				<p className="text-gray-500 text-lg">
					Simulate market scenarios and understand their impacts
				</p>
			</header>
				...
				<div key={cat} className="mb-12">
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
						{SCENARIOS.filter((s) => s.category === cat).map((scenario) => (
							<div
								key={scenario.id}
								className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between hover:shadow-md transition cursor-default"
							>
