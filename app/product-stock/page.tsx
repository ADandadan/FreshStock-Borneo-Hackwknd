...
import type { Ingredient, Product } from "@/constants";
			...
			<div className="flex justify-between items-center mb-10">
				<div>
					...
					<p className="text-gray-500 text-lg">
						Manage your products and calculate costs
					</p>
				</div>
				{!showAddForm && (
					<button
					...
					<div className="mb-8">
						...
						<div className="space-y-3 bg-orange-50/50 p-6 rounded-2xl border border-orange-100">
							{ingredients.map((ing) => (
								<div
									key={ing.id}
									className="grid grid-cols-[2fr,1fr,1fr,1.5fr,auto] gap-4 items-center"
								>
									<input
										placeholder="Name"
										value={ing.name}
					...
					<div className="flex justify-between items-center bg-green-50 p-6 rounded-xl mb-10 border border-green-100">
						<div>
							...
							<p className="text-3xl font-bold text-gray-900">
								RM {calculateTotalCost(ingredients).toFixed(2)}
							</p>
						</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
					{products.length === 0 ? (
						...
						products.map((p) => (
							<div
								key={p.id}
								className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm relative group hover:shadow-md transition-shadow"
							>
								<button
									type="button"
								...
								<p className="text-gray-500 font-medium mb-6">
									{p.inStock} units in stock
								</p>

								<div className="space-y-2.5 text-sm mb-8 border-t border-gray-50 pt-5">
									<p className="font-bold text-gray-500 uppercase tracking-wider text-[10px] mb-3">
										Ingredients
									</p>
								...
								<div className="flex justify-between items-end border-t border-gray-50 pt-5 mt-auto">
									<div>
										...
										<p className="text-lg font-bold text-gray-900">
											RM {calculateTotalCost(p.ingredients ?? []).toFixed(2)}
										</p>
									</div>
									<div>
										<p className="text-[10px] font-bold text-gray-500 uppercase mb-1">
									...
									<div className="text-right">
										...
										<p className="text-lg font-bold text-[#008a00]">
											RM{" "}
											{(
												p.sellingPrice - calculateTotalCost(p.ingredients ?? [])
											).toFixed(2)}
										</p>
									</div>
								</div>
