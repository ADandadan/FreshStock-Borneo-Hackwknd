...
import type { PredictionResult, Product, SaleEntry } from "@/constants";
...
export default function StockPredictionPage() {
	...
	const handleAutoCalculate = () => {
		...
		const freshSales: SaleEntry[] = freshSalesRaw
			? JSON.parse(freshSalesRaw)
			: [];
		...
		<div className="p-10 bg-[#FFFCF6] min-h-screen font-sans">
			<header className="mb-10">
				...
				<p className="text-gray-500 text-lg">
					Forecast stays even after switching sections
				</p>
			</header>
				...
				<div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
					...
					<div className="space-y-4">
						<select
							...
							className="w-full p-3.5 bg-gray-50 rounded-xl outline-none focus:outline-solid"
						...
						<div className="grid grid-cols-2 gap-4">
							<input
								...
								className="p-3.5 bg-gray-50 rounded-xl outline-none focus:outline-solid"
							/>
							<input
								type="number"
								placeholder="Days"
						...
						<button
							...
							className="w-full py-4 bg-orange-500 text-white rounded-xl font-bold transition hover:bg-orange-600"
						>
							Calculate
						</button>
					</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
				{predictions.length > 0 ? (
					predictions.map((res) => (
						<div
							key={res.id}
							className={`p-6 bg-white rounded-2xl border-2 ${res.isShortage ? "border-red-100" : "border-green-100"}`}
						>
							<div className="flex justify-between items-start mb-4">
								...
								<button
									type="button"
									onClick={() => removePrediction(res.id)}
							...
							<div
								className={`p-4 rounded-xl text-center mb-4 ${res.isShortage ? "bg-orange-50" : "bg-green-50"}`}
							>
							...
							{res.isShortage ? (
								<div className="bg-red-600 text-white p-2 rounded-lg text-center text-[10px] font-bold uppercase">
									Restock Required
								</div>
							) : (
								<div className="bg-green-600 text-white p-2 rounded-lg text-center text-[10px] font-bold uppercase">
									Stock Healthy
								</div>
							)}
						</div>
